...
echo "${controllerip} controller" | sudo tee -a /etc/hosts
echo "${worker1ip} worker1" | sudo tee -a /etc/hosts
echo "${worker2ip} worker2" | sudo tee -a /etc/hosts
...
sudo apt-mark hold kubelet kubeadm kubectl # Just to make that these packages do not get upgraded

if [[ $hostname == "controller" ]] || [[ $hostname == "ip-10-0-1-50" ]]
then
